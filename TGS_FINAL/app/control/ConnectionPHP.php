...
if(!$pdo){
    ...
    foreach($fetch as $key=> $value){
        echo $value['ID_USUARIO'] . ' | ' . $value['NOME_USUARIO'] ;
        echo "<br><hr>";
    }*/

    //UPDATE do CRUD
    //$sql_stmt = $pdo->prepare("UPDATE USUARIO SET NOME_USUARIO = 'EllenS222' WHERE ID_USUARIO = 16");
    //$sql_stmt->execute();

    //DELETE do CRUD
    /*$sql_stmt = $pdo->prepare("DELETE FROM USUARIO WHERE ID_USUARIO = ?");
    $id_param = 18;
    $sql_stmt->bindParam('i', $id_param);
    $sql_stmt->execute();*/
